...
{
    public partial class DNCFabricAdditionalEquipmentAddForm : Form
    {
        ...
        public DNCFabricAdditionalEquipmentAddForm()
        {
            InitializeComponent();
            tooltips();
            loadTypes();
        ...
        public DNCFabricAdditionalEquipmentAddForm(string type_id, string equipment_id, string equipment, string price)
        {
            InitializeComponent();
        ...
        public DNCFabricAdditionalEquipmentAddForm(string id, UserControl control)
        {
            InitializeComponent();
        ...
        private async Task<bool> sendNewAdditionalEquipment(string _type_id, string equipment, decimal price)
        {
            ...
            SqlCommand sqlCommand = new SqlCommand($"Insert Into [DNC_Additional_equipment] Values (N'{equipment}', {string.Join(".", Convert.ToString((float)Math.Round(Convert.ToDouble(price), 2, MidpointRounding.AwayFromZero)).Split(','))}, {_type_id});", connection);
        ...
        private async Task<bool> editAdditionalEquipment(string _equipment_id, string _type_id, string equipment, decimal price)
        {
            ...
            SqlCommand sqlCommand = new SqlCommand($"Update [DNC_Additional_equipment] Set [Equipment] = N'{equipment}', [Type_id] = {_type_id}, [Price] = {string.Join(".", Convert.ToString((float)Math.Round(Convert.ToDouble(price), 2, MidpointRounding.AwayFromZero)).Split(','))} Where [Equipment_id] = {_equipment_id};", connection);
        ...
        private async Task<bool> deleteEquipment(string _equipment_id)
        {
            ...
            SqlCommand sqlCommand = new SqlCommand($"Delete From [DNC_Additional_equipment] Where [Equipment_id] = {_equipment_id};", connection);
