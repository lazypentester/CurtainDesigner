...
{
    public partial class DNCFabricCategoryAddForm : Form
    {
        ...
        public DNCFabricCategoryAddForm()
        {
            InitializeComponent();
            tooltips();
            loadTypes();
        ...
        public DNCFabricCategoryAddForm(string category_id, string type_id, string subtype_id, string category, decimal price)
        {
            InitializeComponent();
        ...
        public DNCFabricCategoryAddForm(string id, UserControl control)
        {
            InitializeComponent();
        ...
        private async Task<bool> sendNewCategory(string _type_id, string _subtype_id, string category, decimal price)
        {
            ...
            SqlCommand sqlCommand = new SqlCommand($"Insert Into [DNC_category] Values (N'{category}', {string.Join(".", Convert.ToString((float)Math.Round(Convert.ToDouble(price), 2, MidpointRounding.AwayFromZero)).Split(','))}, {_type_id}, {_subtype_id});", connection);
        ...
        private async Task<bool> editCategory(string _category_id, string _type_id, string _subtype_id, string category, decimal price)
        {
            ...
            SqlCommand sqlCommand = new SqlCommand($"Update [DNC_category] Set Category = N'{category}', Price = {string.Join(".", Convert.ToString((float)Math.Round(Convert.ToDouble(price), 2, MidpointRounding.AwayFromZero)).Split(','))} Where Category_id = {_category_id} and Type_id = {_type_id} and Subtype_id = {_subtype_id};", connection);
        ...
        private async Task<bool> deleteCategory(string _category_id)
        {
            ...
            SqlCommand sqlCommand = new SqlCommand($"Delete From [DNC_category] Where [DNC_category].[Category_id] = {_category_id};", connection);
