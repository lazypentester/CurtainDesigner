...
{
    public partial class MCTypeAddForm : Form
    {
        ...
        public MCTypeAddForm()
        {
            InitializeComponent();
            tooltips();
        }

        public MCTypeAddForm(string id, string type_name, decimal price)
        {
            InitializeComponent();
        ...
        public MCTypeAddForm(string id, UserControl control)
        {
            InitializeComponent();
        ...
        private async void removeType(string id, UserControl control)
        {
            ...
            if (send)
            {
                ...
                (control as UserControls.UCSettingsMosquitoCurtain.UserControlCurt_TypeMC).load_types();
            }
            else
        ...
        private async Task<bool> deleteType(string id)
        {
            ...
            SqlCommand sqlCommand = new SqlCommand($"Delete From [MC_types] Where [MC_types].[Type_id] = {id};", connection);
        ...
        private async Task<bool> sendNewType(string type_name, decimal price)
        {
            ...
            SqlCommand sqlCommand = new SqlCommand($"Insert Into [MC_types] Values (N'{type_name}', {string.Join(".", Convert.ToString((float)Math.Round(Convert.ToDouble(price), 2, MidpointRounding.AwayFromZero)).Split(','))});", connection);
        ...
        private async Task<bool> editType(string id, string type_name, decimal price)
        {
            ...
            SqlCommand sqlCommand = new SqlCommand($"Update [MC_types] Set [MC_types].[Type_name] = N'{type_name}', [MC_types].[Price] = {string.Join(".", Convert.ToString((float)Math.Round(Convert.ToDouble(price), 2, MidpointRounding.AwayFromZero)).Split(','))} Where [MC_types].[Type_id] = {id};", connection);
