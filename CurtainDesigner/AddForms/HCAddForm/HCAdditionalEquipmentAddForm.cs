...
{
    public partial class HCAdditionalEquipmentAddForm : Form
    {
        ...
        public HCAdditionalEquipmentAddForm()
        {
            InitializeComponent();
            tooltips();
        }

        public HCAdditionalEquipmentAddForm(string equipment_id, string equipment, string price)
        {
            InitializeComponent();
        ...
        public HCAdditionalEquipmentAddForm(string id, UserControl control)
        {
            InitializeComponent();
        ...
        private async Task<bool> sendNewAdditionalEquipment(string equipment, decimal price)
        {
            ...
            SqlCommand sqlCommand = new SqlCommand($"Insert Into [HC_Additional_equipment] Values (N'{equipment}', {string.Join(".", Convert.ToString((float)Math.Round(Convert.ToDouble(price), 2, MidpointRounding.AwayFromZero)).Split(','))});", connection);
        ...
        private async Task<bool> editAdditionalEquipment(string _equipment_id, string equipment, decimal price)
        {
            ...
            SqlCommand sqlCommand = new SqlCommand($"Update [HC_Additional_equipment] Set [Equipment] = N'{equipment}', [Price] = {string.Join(".", Convert.ToString((float)Math.Round(Convert.ToDouble(price), 2, MidpointRounding.AwayFromZero)).Split(','))} Where [Equipment_id] = {_equipment_id};", connection);
        ...
        private async void removeEquipment(string id, UserControl control)
        {
            ...
            if (send)
            {
                ...
                (control as UserControls.UCSettingsHorizontalCurtain.UserControlCurt_AdditionalEquipmentHC).load_equipments();
            }
            else
        ...
        private async Task<bool> deleteEquipment(string _equipment_id)
        {
            ...
            SqlCommand sqlCommand = new SqlCommand($"Delete From [HC_Additional_equipment] Where [Equipment_id] = {_equipment_id};", connection);
