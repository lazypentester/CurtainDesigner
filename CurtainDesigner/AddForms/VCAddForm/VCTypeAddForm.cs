...
{
    public partial class VCTypeAddForm : Form
    {
        ...
        public VCTypeAddForm()
        {
            InitializeComponent();
            tooltips();
        }

        public VCTypeAddForm(string id, string type_name, decimal price)
        {
            InitializeComponent();
        ...
        public VCTypeAddForm(string id, UserControl control)
        {
            InitializeComponent();
        ...
        private async void removeType(string id, UserControl control)
        {
            ...
            if (send)
            {
                ...
                (control as UserControls.UCSettingsVerticalCurtain.UserControlCurt_TypeVC).load_types();
            }
            else
        ...
        private async Task<bool> deleteType(string id)
        {
            ...
            SqlCommand sqlCommand = new SqlCommand($"Delete From [VC_types] Where [VC_types].[Type_id] = {id};", connection);
        ...
        private async Task<bool> sendNewType(string type_name, decimal price)
        {
            ...
            SqlCommand sqlCommand = new SqlCommand($"Insert Into [VC_types] Values (N'{type_name}', {string.Join(".", Convert.ToString((float)Math.Round(Convert.ToDouble(price), 2, MidpointRounding.AwayFromZero)).Split(','))});", connection);
        ...
        private async Task<bool> editType(string id, string type_name, decimal price)
        {
            ...
            SqlCommand sqlCommand = new SqlCommand($"Update [VC_types] Set [VC_types].[Type_name] = N'{type_name}', [VC_types].[Price] = {string.Join(".", Convert.ToString((float)Math.Round(Convert.ToDouble(price), 2, MidpointRounding.AwayFromZero)).Split(','))} Where [VC_types].[Type_id] = {id};", connection);
